...
bool isGod( std::vector<std::vector<char> > &mapa, Ping &temp , map< Ping , int > &pas , int x , int y)
{
  ...
  if( pas.find(temp) != pas.end() )
    return false;
  pas[temp]=0;
  if( temp.a.first < 0 || temp.b.first < 0 )
    return false;
  if( temp.a.first >= x || temp.b.first >= x )
    return false;
  if( temp.a.second < 0 || temp.b.second < 0 )
    return false;
  if( temp.a.second >= y || temp.b.second >= y )
    return false;
  if( mapa[temp.a.first][temp.a.second] != '.' || mapa[temp.b.first][temp.b.second] != '.' )
    return false;
  return true;
}

void imprimir( std::vector<std::vector<char> > &mapa,Ping &ini , int x , int y )
{
  string ab;
  getline(cin,ab);
  foi( i ,  0 , x )
